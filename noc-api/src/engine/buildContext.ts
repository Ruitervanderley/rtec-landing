 ...
 */
export async function buildDiagnosticContext(): Promise<DiagnosticContext | null> {
  ...
  const deviceToServices = new Map<string, Array<{ id: string; nome: string }>>();
  ...
  const eventsWithMeta = rows.map((r) => {
    ...
    return {
      id: r.eventId,
      deviceId: r.deviceId,
      tipo: r.tipo as DiagnosticContext['events'][0]['tipo'],
      valor: r.valor,
  ...
  const offlineByService = new Map<
    string,
    { deviceIds: string[]; serviceNome: string }
  >();
  const offlineByArea = new Map<
    string,
    { deviceIds: string[]; areaNome: string; siteNome: string }
  >();
  const offlineDeviceIds = new Set<string>();
  const deviceHasOffline = new Set<string>();
  const deviceHasHighLatency = new Set<string>();
  ...
  const gatewayOfflineSites: DiagnosticContext['gatewayOfflineSites'] = [];
