...
const DEVICE_TYPES = ['router', 'camera', 'switch', 'server'] as const;
...
router.post('/', async (req: Request, res: Response) => {
  try {
    const { nome, tipo, local, ip, cliente_id: clienteId } = req.body as {
      nome?: string;
      tipo?: string;
      local?: string;
      ip?: string;
    ...
    const [device] = await db
      ...
      .values({
        nome,
        tipo: tipo as (typeof DEVICE_TYPES)[number],
        local,
        ip,
