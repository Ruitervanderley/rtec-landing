...
router.post('/', async (req: Request, res: Response) => {
  try {
    const { nome, criticidade } = req.body as { nome?: string; criticidade?: string };
    ...
    if (
      ...
      || !CRITICIDADES.includes(criticidade as (typeof CRITICIDADES)[number])
    ) {
      res.status(400).json({
    ...
    const [service] = await db
      ...
      .values({
        nome: nome.trim(),
        criticidade: criticidade as (typeof CRITICIDADES)[number],
      })
      .returning();
...
router.get('/:id', async (req: Request, res: Response) => {
  try {
    ...
    const incidentDeviceIds = new Map<string, string[]>();
...
router.delete('/:id/devices/:deviceId', async (req: Request, res: Response) => {
  try {
    ...
    await db
      ...
      .where(
        and(
          eq(serviceDevices.serviceId, serviceId),
          eq(serviceDevices.deviceId, deviceId),
        ),
      );
    res.status(204).send();
  } catch (err) {
