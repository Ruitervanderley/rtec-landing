...
export class OpsJobRunner {
  ...
  private readonly state: OpsRuntimeState;
  ...
  public getState(): OpsRuntimeState {
    return {
      ...this.state,
    };
  }

  private async scanOfflineDevices(): Promise<void> {
    try {
      ...
      const rows = await db
        ...
        .where(
          and(
            ...
            lt(tenantDevices.lastSeenAt, threshold),
          ),
        );
