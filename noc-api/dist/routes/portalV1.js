...
async function getRecentSessions(tenantId, limit) {
    const rows = await db.execute(sql `
    select
      s.session_guid,
      s.speaker_name,
  ...
  `);
    return rows.rows.map(row => ({
        sessionGuid: String(row.session_guid ?? ''),
        speakerName: String(row.speaker_name ?? ''),
        startedAtUtc: toIsoString(row.started_at_utc),
        endedAtUtc: toIsoString(row.ended_at_utc),
        plannedSeconds: toNumber(row.planned_seconds),
        elapsedSeconds: toNumber(row.elapsed_seconds),
...
async function getSpeakerUsage(props) {
  ...
  `);
    return rows.rows.map(row => ({
        averageElapsedSeconds: Math.round(Number(row.average_elapsed_seconds ?? 0)),
        speakerName: String(row.speaker_name ?? ''),
        totalElapsedSeconds: toNumber(row.total_elapsed_seconds),
...
export function createPortalV1Router(config) {
    const router = Router();
    router.get('/me', requirePortalUser(config), async (req, res) => {
    ...
    router.get('/tenants/:slug/overview', requirePortalUser(config), async (req, res) => {
        try {
    ...
    router.get('/tenants/:slug/sessions', requirePortalUser(config), async (req, res) => {
        try {
            const slug = normalizeSlug(req.params.slug ?? '');
            if (!slug) {
                res.status(400).json({ error: 'SLUG_REQUIRED' });
    ...
    router.get('/tenants/:slug/speaker-usage', requirePortalUser(config), async (req, res) => {
        try {
            const slug = normalizeSlug(req.params.slug ?? '');
            if (!slug) {
                res.status(400).json({ error: 'SLUG_REQUIRED' });
    ...
    router.get('/tenants/:slug/sessions/:sessionGuid/audit-logs', requirePortalUser(config), async (req, res) => {
        try {
            const slug = normalizeSlug(req.params.slug ?? '');
        ...
        limit 1
      `);
            const sessionRow = (sessionRows.rows[0] ?? null);
            if (!sessionRow) {
      ...
      `);
            res.json({
                logs: logRows.rows.map(row => ({
                    ...
                    elapsedSeconds: toNumber(row.elapsed_seconds),
                    eventAtUtc: toIsoString(row.event_at_utc),
                    eventType: String(row.event_type ?? ''),
                    id: toNumber(row.id),
                    remainingSeconds: toNumber(row.remaining_seconds),
                })),
                session: {
...
//# sourceMappingURL=portalV1.js.map