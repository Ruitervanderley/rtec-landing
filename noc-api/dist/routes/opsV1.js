...
function parseBackupStatus(success) {
    if (typeof success === 'boolean') {
        return success ? 'UPLOADED' : 'FAILED';
    }
    return 'FAILED';
}
function ensureRateLimit(key, limit, windowMs, res) {
...
function selectTenantProvisionOwner(users) {
...
async function getTenantAgentSummary(tenantId) {
...
export function createOpsV1Router(options) {
    const router = Router();
    router.post('/device/provision', async (req, res) => {
        ...
        try {
            ...
            await db
                ...
                .where(and(eq(deviceApiTokens.deviceFk, devicePk), isNull(deviceApiTokens.revokedAt)));
    ...
    router.post('/device/heartbeat', requireDeviceToken(), async (req, res) => {
    ...
    router.post('/backups/request-upload', requireDeviceToken(), async (req, res) => {
        ...
        try {
            const { backupType, sessionGuid, fileName, sizeBytes, sha256 } = req.body;
    ...
    router.post('/backups/complete', requireDeviceToken(), async (req, res) => {
        if (!req.opsDevice) {
            res.status(401).json({ error: 'UNAUTHORIZED_DEVICE' });
            return;
        }
        ...
        try {
            const { backupId, success, sizeBytes, sha256, errorMessage } = req.body;
            ...
            const current = await db
                ...
                .from(deviceBackups)
                .where(and(eq(deviceBackups.id, backupId), eq(deviceBackups.deviceFk, req.opsDevice.devicePk)))
                .limit(1);
            if (current.length === 0) {
    ...
    router.get('/admin/overview', requireAdminToken(options.config), async (_req, res) => {
        ...
        `);
            const operationalTenants = tenantSummaryResult.rows
                .map(row => buildTenantOperationalSummary(row))
                .sort((left, right) => {
                const statusWeight = { critical: 0, degraded: 1, healthy: 2 };
    ...
    router.get('/admin/backups', requireAdminToken(options.config), async (req, res) => {
        try {
            const limitRaw = Number(req.query.limit ?? 200);
            const limit = Number.isFinite(limitRaw) ? Math.max(1, Math.min(1000, Math.trunc(limitRaw))) : 200;
            const tenantId = typeof req.query.tenantId === 'string' ? req.query.tenantId.trim() : null;
    ...
    router.get('/admin/tenants', requireAdminToken(options.config), async (_req, res) => {
        try {
            const rows = await db.execute(sql `
        select
          t.id,
    ...
    router.post('/admin/tenants/:id/users', requireAdminToken(options.config), async (req, res) => {
        try {
            ...
            const { email, password, display_name, is_admin, valid_until } = req.body;
    ...
    router.patch('/admin/tenants/:id/users/:userId', requireAdminToken(options.config), async (req, res) => {
        try {
            const tenantId = String(req.params.id ?? '').trim();
            ...
            const { display_name, is_admin, valid_until } = req.body;
            const updates = {};
            ...
            const profileTenantId = String(existingProfile.tenant_id ?? '');
    ...
    router.post('/admin/tenants/:id/users/:userId/reset-password', requireAdminToken(options.config), async (req, res) => {
        try {
            const tenantId = String(req.params.id ?? '').trim();
            const userId = String(req.params.userId ?? '').trim();
            if (!tenantId || !userId) {
    ...
    router.delete('/admin/tenants/:id/users/:userId', requireAdminToken(options.config), async (req, res) => {
        try {
            const tenantId = String(req.params.id ?? '').trim();
            const userId = String(req.params.userId ?? '').trim();
            if (!tenantId || !userId) {
    ...
    router.post('/admin/tenants/:id/agent/provision', requireAdminToken(options.config), async (req, res) => {
        try {
            const tenantId = String(req.params.id ?? '').trim();
    ...
    router.post('/admin/devices/:id/token', requireAdminToken(options.config), async (req, res) => {
        try {
